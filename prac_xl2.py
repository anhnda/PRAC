"""
Wanda Pruning with Moderate Weight Correction - XL Version (Moment Ratio)

Combines Wanda pruning with moderate weight correction using moment ratio:
1. Apply Wanda pruning (Score_ij = |W_ij| * ||X_j||_2)
2. Select "moderate" positions based on |JS_mean(X_j)|
3. Correct weights using moment ratio: ΔW_ij = -Error_i × E[X_j] / (E[X_j²] + λ)
4. Fully vectorized across all output channels

Key Features:
- L2 norm salience for Wanda scoring
- Moment ratio for adaptive weight correction
...
Usage:
    python prac_xl2.py --sparsity 0.5 --percent-change 0.05 --lambda-reg 1e-6
"""
...
class WandaPrunerWithCorrection:
    """
    Wanda Pruning with Moderate Weight Correction using moment ratio.
    ...
    def __init__(self, model, tokenizer, device="cuda", sparsity=0.5,
                 ...
                 max_correction_magnitude=0.05, lambda_reg=1e-6):
        self.model = model
        ...
        print(f"\n[Wanda Pruner with Moderate Correction - XL Version (Moment Ratio)]")
        ...
        if use_correction:
            print(f"  Moderate correction: ENABLED (Moment Ratio)")
            print(f"    - Formula: ΔW_ij = -Error_i × E[X_j] / (E[X_j²] + λ)")
    ...
    def get_activation_stats(self, name):
        """
        Compute L2 salience, second moment, and James-Stein mean from pre-accumulated statistics.

        Memory efficient: Uses O(hidden_dim) instead of O(samples × hidden_dim).

        Returns:
            tuple: (l2_salience, second_moment, js_mean, raw_mean)
        """
        ...
        second_moment = stats['l2_sum'] / stats['count']  # E[X_j²]
        raw_mean = stats['mean_sum'] / stats['count']      # E[X_j]

        # Apply James-Stein estimator
        js_mean = compute_james_stein_mean(raw_mean)

        return l2_salience, second_moment, js_mean, raw_mean
    ...
    def correct_weights_vectorized(self, W_orig, W_pruned, raw_mean, second_moment,
        ...
        Args:
            ...
            raw_mean: First moment E[X_j] [in_features]
            second_moment: Second moment E[X_j²] [in_features]
        ...
        raw_mean_f32 = raw_mean.float()
        second_moment_f32 = second_moment.float()

        # Compute reconstruction errors for all channels
        # error[i] = (W_pruned[i,:] - W_orig[i,:]) · E[X]
        W_diff = W_pruned_f32 - W_orig_f32
        errors = torch.matmul(W_diff, raw_mean_f32)  # [out_features]
        ...
        for j in selected_positions:
            # Moment ratio: E[X_j] / (E[X_j²] + λ)
            adaptive_weight = raw_mean_f32[j] / (second_moment_f32[j] + self.lambda_reg)
        ...
        errors_after = torch.matmul(W_corrected.float() - W_orig_f32, raw_mean_f32)
    ...
    def prune_layer(self, name, module, debug=False):
        """
        Prune layer with optional moderate weight correction using moment ratio.
        """
        if name not in self.activation_stats:
        ...
        l2_salience, second_moment, js_mean, raw_mean = self.get_activation_stats(name)
        ...
        if debug:
            ...
            print(f"    Second moment range: [{second_moment.min():.6f}, {second_moment.max():.6f}]")
            print(f"    Raw mean range: [{raw_mean.min():.6f}, {raw_mean.max():.6f}]")
        ...
        second_moment_device = second_moment.to(self.device).to(W.dtype)
        js_mean_device = js_mean.to(self.device).to(W.dtype)
        raw_mean_device = raw_mean.to(self.device).to(W.dtype)
        ...
        self.layer_stats[name] = {
            ...
            'raw_mean_mean': raw_mean.mean().item(),
            'second_moment_mean': second_moment.mean().item(),
        ...
        del W_device, l2_salience_device, second_moment_device, js_mean_device, raw_mean_device, W_pruned, W_final
    ...
    def prune_model_sequential(self, calibration_data, n_samples=500, layer_batch_size=16):
        """Batched sequential pruning with correction using moment ratio."""
        print("\n" + "=" * 80)
        print("BATCHED SEQUENTIAL PRUNING WITH MOMENT RATIO CORRECTION (XL Version)")
        print("=" * 80)
        ...
        if self.layer_stats:
            ...
            if self.use_correction:
                ...
                if corrected_layers:
                    ...
                    print(f"\nMoment ratio correction statistics ({len(corrected_layers)} layers):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Wanda Pruning with Moment Ratio Weight Correction (XL Version)",
    ...
    parser.add_argument("--lambda-reg", type=float, default=1e-6,
                       help="Lambda regularization for moment ratio (default 1e-6)")
    ...
    print("=" * 80)
    print("Wanda Pruning with Moment Ratio Weight Correction (XL Version)")
    ...
    if args.use_correction:
        print(f"  Correction method: Moment ratio (E[X] / (E[X²] + λ))")
