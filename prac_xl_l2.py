"""
Wanda Pruning with Moderate Weight Correction - XL Version (L2 Norm Distribution)

Combines Wanda pruning with moderate weight correction using L2 norm for error distribution:
1. Apply Wanda pruning (Score_ij = |W_ij| * ||X_j||_2)
2. Select "moderate" positions based on |JS_mean(X_j)|
3. Correct weights at moderate positions using L2 norm (energy) distribution
4. Fully vectorized across all output channels

Key Features:
- L2 norm salience for Wanda scoring
- L2 norm (energy) for error distribution: ΔW_ij = -Error_i × sign(W_ij) × Salience_j / Σ(Salience_k)
...
Usage:
    python prac_xl_l2.py --sparsity 0.5 --percent-change 0.05
"""
...
class WandaPrunerWithCorrection:
    """
    Wanda Pruning with Moderate Weight Correction using L2 norm distribution.
    ...
    def __init__(self, model, tokenizer, device="cuda", sparsity=0.5,
        ...
        print(f"\n[Wanda Pruner with Moderate Correction - XL Version (L2 Distribution)]")
        ...
        if use_correction:
            print(f"  Moderate correction: ENABLED (L2 norm distribution)")
            print(f"    - Formula: ΔW_ij = -Error_i × sign(W_ij) × Salience_j / Σ(Salience_k)")
    ...
    @torch.no_grad()
    def correct_weights_vectorized(self, W_orig, W_pruned, js_mean, l2_salience,
                                   selected_positions, prune_mask, debug=False):
        """
        Vectorized weight correction using L2 norm (energy) distribution.

        Key: Only corrects SURVIVING moderate weights (not pruned ones).
        Uses L2 norm for distributing the error instead of L1 norm.

        For each output channel i:
            error[i] = (W_pruned[i,:] - W_orig[i,:]) · js_mean

        For each moderate position j (among survivors):
            ΔW[i,j] = -error[i] × sign(W[i,j]) × salience[j] / Σ(salience[selected])
        ...
        Args:
            ...
            js_mean: James-Stein mean [in_features]
            l2_salience: L2 salience [in_features]
        ...
        l2_salience_f32 = l2_salience.float()
        ...
        # Sum of L2 salience at selected positions (energy-based normalization)
        sum_salience = l2_salience_f32[selected_positions].sum()

        if sum_salience < 1e-10:
            if debug:
                print(f"    ⚠️ Sum of salience too small, no correction")
        ...
        # ΔW[:,j] = -errors × sign(W[:,j]) × salience[j] / sum_salience
        ...
        for j in selected_positions:
            # Use sign of the weight itself (not js_mean)
            sign_W_j = torch.sign(W_pruned_f32[:, j])

            # L2 norm distribution: use salience as the distribution weight
            delta_W_j = -errors * sign_W_j * l2_salience_f32[j] / sum_salience
    ...
    def prune_layer(self, name, module, debug=False):
        """
        Prune layer with optional moderate weight correction using L2 norm distribution.
        """
        if name not in self.activation_stats:
        ...
        if self.use_correction:
            ...
            # Correct weights (only surviving ones) using L2 norm distribution
            if knee_info is not None and len(selected_positions) > 0:
                W_final, correction_stats = self.correct_weights_vectorized(
                    W_device, W_pruned, js_mean_device, l2_salience_device,
    ...
    def prune_model_sequential(self, calibration_data, n_samples=500, layer_batch_size=16):
        """Batched sequential pruning with correction using L2 norm distribution."""
        print("\n" + "=" * 80)
        print("BATCHED SEQUENTIAL PRUNING WITH L2 NORM CORRECTION (XL Version)")
        print("=" * 80)
        ...
        if self.layer_stats:
            ...
            if self.use_correction:
                ...
                if corrected_layers:
                    ...
                    print(f"\nL2 norm correction statistics ({len(corrected_layers)} layers):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Wanda Pruning with L2 Norm Weight Correction (XL Version)",
    ...
    parser.add_argument("--output-dir", type=str, default="./pruned_models/model_prac_xl_l2",
    ...
    print("=" * 80)
    print("Wanda Pruning with L2 Norm Weight Correction (XL Version)")
    ...
    if args.use_correction:
        print(f"  Correction method: L2 norm (energy) distribution")
